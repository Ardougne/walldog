...
#walldog talks woa
	...
	c = ('fuck you walldog', 'fuck walldog', 'i hate walldog', 'bad bot')
	lower = str.lower(message.content)
	...
	lower = str.lower(message.content)
	if lower == (e):
		await message.channel.send('i love asdas')

	f = ('disney')
	lower = str.lower(message.content)
	if lower == (f):
		await message.channel.send('You mean benevolent overlord Disney chan(nel) sama?')	

	g = ('<@439205512425504771>', 'notsobot', 'notsosuper',)
	lower = str.lower(message.content)
	if lower.startswith(g):
		await message.channel.send('NotSoSuper more like NotSoStraight')
